...
#
#   1. ref_cloth_mask  — worn garment pixels (your Segformer)
#   2. parsed_labels   — body part map (SCHP) — arms only
#   3. identity_mask   — face + hands (SAM)   — never greyed
#
# Trouser handling is intentionally absent.
# Length difference + trouser inpainting is handled separately
# at inference time via SD inpainting.
#
# Outputs
# ───────
#   agnostic/         *_agnostic.jpg      — grey canvas person image
#   agnostic_mask/    *_mask.png          — canvas mask (cloth + arms)
#   agnostic_mask/    *_inpaint_mask.png  — tight mask for CatVTON (cloth only)
# """
#
# import os
# import cv2
...
#
# # NEW: Kernel for expanding the masks to create an inpainting buffer
# DILATE_KERNEL = np.ones((11, 11), np.uint8)
# EXPAND_ITER = 1  # Increase to expand further
#
# FEATHER_RADIUS = 7  # must be odd
#
#
# # ==========================================
# # HELPERS
...
#
#             # ---> NEW: Expand the mask slightly to cover garment seams
...
#
#             # ---> NEW: Ensure the inpaint mask also includes the expansion buffer
#             tight_mask = cv2.dilate(tight_mask, DILATE_KERNEL, iterations=EXPAND_ITER)
#
#             tight_mask[identity_mask == 255] = 0
#
#             # ── AGNOSTIC IMAGE ────────────────────────────────────────────────
#             soft_mask = cv2.GaussianBlur(canvas_mask, (FEATHER_RADIUS, FEATHER_RADIUS), 0)
#             alpha = soft_mask.astype(np.float32) / 255.0
#             alpha_3ch = np.expand_dims(alpha, axis=2)
#             agnostic = (img.astype(np.float32) * (1.0 - alpha_3ch) + 128.0 * alpha_3ch).astype(np.uint8)
#
#             # ── SAVE ──────────────────────────────────────────────────────────
